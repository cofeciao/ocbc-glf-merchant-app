...
const ContactInformation: React.FC<IContactInformation.IProps> = forwardRef(
  ({ handleCallAPI }, ref) => {
      ...
      return (
        <div className={cx("group-item")}>
          ...
          {Array.isArray(content) ? (
            content.map(
              (item: IContactInformation.IDirectors, index: number) => (
              ...
              <Grid container spacing={1}>
                <Grid item xs={4}>
                  {renderItemInformation(
                    "Business name",
                    dataArca.businessName
                  )}
                </Grid>
                <Grid item xs={4}>
                  {renderItemInformation(
                    "Unique Entity Number (UEN)",
                    dataArca.uniqueEntityNumber
                  )}
                </Grid>
                <Grid item xs={2}></Grid>
                <Grid item xs={4}>
                  {renderItemInformation("Entity type", dataArca.entityType)}
                </Grid>
                <Grid item xs={4}>
                  {renderItemInformation(
                    "Nature of business",
                    dataArca.natureOfBusiness
                  )}
                </Grid>
                <Grid item xs={2}></Grid>
                <Grid item xs={4}>
                  {renderItemInformation(
                    "Registered address",
                    dataArca.registeredAddress
                  )}
                </Grid>
                <Grid item xs={4}>
                  {renderItemInformation(
                    "Mailing address",
                    dataArca.mailingAddress
                  )}
                </Grid>
                <Grid item xs={2}></Grid>
                <Grid item xs={4}>
