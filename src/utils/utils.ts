...
export const updateDataListRadio = (value, listRadio) => {
...
export const scrollToError = function () {
  setTimeout(() => {
    ...
    if (errorElements.length) {
      ...
      const y =
        firstElement.getBoundingClientRect().top + window.pageYOffset + yOffset;
...
export const moneyDisplayFormat = (num, precision) => {
  ...
  const formattedDollars = arrDollarCents[0].replace(
    /(\d)(?=(\d{3})+(?!\d))/g,
    "$1,"
  );
  if (precision === 0) {
  ...
  return `${formattedDollars}.${
    (arrDollarCents[1] &&
      (arrDollarCents[1].toString().length >= precision
        ? arrDollarCents[1]
        : arrDollarCents[1] +
          "0".repeat(precision - arrDollarCents[1].length))) ||
    "0".repeat(precision)
  }`;
};
...
export const formatDataHdbownership = (formLandingData: any) => {
  ...
  if (
    formLandingData &&
    formLandingData.myInfo &&
    formLandingData.myInfo.hdbownership &&
    _.isArray(formLandingData.myInfo.hdbownership)
  ) {
...
export const mapCpfWithDrawDetail = (formRedux: any) => {
  const data = [];
  const dataCpfWithDrawDetail =
    formRedux.myInfo &&
    formRedux.myInfo.cpfhousingwithdrawal &&
    formRedux.myInfo.cpfhousingwithdrawal.withdrawaldetails &&
    formRedux.myInfo.cpfhousingwithdrawal.withdrawaldetails[0]
      ? formRedux.myInfo.cpfhousingwithdrawal.withdrawaldetails
      : [];
  if (dataCpfWithDrawDetail.length > 0) {
    for (var i = 0; i < dataCpfWithDrawDetail.length; i++) {
      ...
      data.push({
        BlockNo:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].address &&
            dataCpfWithDrawDetail[i].address.block &&
            dataCpfWithDrawDetail[i].address.block.value) ||
          "",
        Street:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].address &&
            dataCpfWithDrawDetail[i].address.street &&
            dataCpfWithDrawDetail[i].address.street.value) ||
          "",
        Floor:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].address &&
            dataCpfWithDrawDetail[i].address.floor &&
            dataCpfWithDrawDetail[i].address.floor.value) ||
          "",
        Unit:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].address &&
            dataCpfWithDrawDetail[i].address.unit &&
            dataCpfWithDrawDetail[i].address.unit.value) ||
          "",
        PostalCode:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].address &&
            dataCpfWithDrawDetail[i].address.postal &&
            dataCpfWithDrawDetail[i].address.postal.value) ||
          "",
        BuildingName:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].address &&
            dataCpfWithDrawDetail[i].address.building &&
            dataCpfWithDrawDetail[i].address.building.value) ||
          "",
        ...
        AccruedInterestAmt:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].accruedinterestamt &&
            Number(dataCpfWithDrawDetail[i].accruedinterestamt.value)) ||
          0,
        PrincipalWithdrawalAmt:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].principalwithdrawalamt &&
            Number(dataCpfWithDrawDetail[i].principalwithdrawalamt.value)) ||
          0,
        TotalCPFAllowedForProperty:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].totalamountofcpfallowedforproperty &&
            Number(
              dataCpfWithDrawDetail[i].totalamountofcpfallowedforproperty.value
            )) ||
          0,
        Monthlyinstalmentamt:
          (dataCpfWithDrawDetail &&
            dataCpfWithDrawDetail[i] &&
            dataCpfWithDrawDetail[i].monthlyinstalmentamt &&
            Number(dataCpfWithDrawDetail[i].monthlyinstalmentamt.value)) ||
          0,
        CreatedBy: "",
        UpdatedBy: "",
        CreatedDate: moment().format(),
