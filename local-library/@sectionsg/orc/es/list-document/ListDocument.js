import _slicedToArray from "@babel/runtime/helpers/esm/slicedToArray";

/** *
 * LIST DOCUMENT
 * child
 *  >> content
 */
// import modules
import DescriptionIcon from '@material-ui/icons/Description';
import clsx from 'clsx';
import PropTypes from 'prop-types';
import React, { useEffect, useState } from 'react'; // import icon

/* babel-plugin-inline-import '../assets/images/icon-info-blue.svg' */
var myInfoBlue = "data:image/svg+xml;base64,";

/* babel-plugin-inline-import '../assets/images/icon-info.svg' */
var myInfo = "data:image/svg+xml;base64,"; // import style

import useStyles from './styles'; // import child component

import Link from '../link'; // render UI

var ListDocument = function ListDocument(_ref) {
  var dataListDocument = _ref.dataListDocument,
      className = _ref.className,
      handleDialog = _ref.handleDialog;
  var classes = useStyles();

  var _useState = useState(dataListDocument),
      _useState2 = _slicedToArray(_useState, 2),
      listDataDocument = _useState2[0],
      setListDataDocument = _useState2[1]; // list data render


  useEffect(function () {
    setListDataDocument(dataListDocument);
  }, [dataListDocument]);
  return /*#__PURE__*/React.createElement("div", {
    className: clsx(classes.listDocument, className)
  }, /*#__PURE__*/React.createElement("ul", null, listDataDocument.length > 0 && listDataDocument.map(function (item, idx) {
    return /*#__PURE__*/React.createElement("li", {
      key: idx
    }, /*#__PURE__*/React.createElement(DescriptionIcon, null), item.label, item.link && /*#__PURE__*/React.createElement("span", {
      className: classes.linkHref
    }, /*#__PURE__*/React.createElement(Link, {
      href: "/",
      classHover: "linkUnderline",
      target: ""
    }, item.link)), item.popupName && /*#__PURE__*/React.createElement("span", {
      className: classes.iconPopup,
      onClick: function onClick() {
        return handleDialog(item.popupName);
      },
      "aria-hidden": "true"
    }, /*#__PURE__*/React.createElement("img", {
      src: myInfo,
      alt: "myInfo-logo",
      className: clsx(classes.icons, 'icon-black')
    }), /*#__PURE__*/React.createElement("img", {
      src: myInfoBlue,
      alt: "myInfo-logo",
      className: clsx(classes.icons, 'icon-blue')
    })));
  })));
};

ListDocument.defaultProps = {
  dataListDocument: [],
  className: '',
  handleDialog: function handleDialog() {}
};
ListDocument.propTypes = {
  dataListDocument: PropTypes.array,
  className: PropTypes.string,
  handleDialog: PropTypes.func
};
export default ListDocument;